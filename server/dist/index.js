...
app.post('/assets/register', upload.array('files', 10), async (req, res) => {
    try {
        const { assetType, assetName, valueUSD, owner, isLocked: isLockedRaw } = req.body;
        const files = req.files || [];
        if (!files.length)
            return res.status(400).json({ error: 'files missing' });
        if (!assetName || typeof assetName !== 'string')
            return res.status(400).json({ error: 'assetName required' });
        ...
        const pk = process.env.BACKEND_PRIVATE_KEY?.startsWith('0x') ? process.env.BACKEND_PRIVATE_KEY : `0x${process.env.BACKEND_PRIVATE_KEY}`;
        const rpcUrl = process.env.SEPOLIA_RPC_URL;
        const vaAddr = process.env.RWA_VERIFIED_ASSETS;
        if (!pk || pk.length < 10)
            throw new Error('Missing BACKEND_PRIVATE_KEY');
        if (!rpcUrl)
            throw new Error('Missing SEPOLIA_RPC_URL');
        if (!vaAddr)
            throw new Error('Missing RWA_VERIFIED_ASSETS');
        ...
        try {
            ...
            id = Array.isArray(existing) ? existing.length : 0;
        }
        catch {
        ...
        const ownerAddr = (owner && /^0x[0-9a-fA-F]{40}$/.test(owner)) ? owner : account.address;
        ...
        const encoded = encodeAbiParameters(parseAbiParameters('uint8, string, uint256, bool, uint256, address'), [
            Number(assetType || 0),
...
app.post('/assets/deregister', async (req, res) => {
    try {
        const { owner, assetId } = req.body;
        if (!owner || !assetId)
            return res.status(400).json({ error: 'owner and assetId required' });
        const pk = process.env.BACKEND_PRIVATE_KEY?.startsWith('0x') ? process.env.BACKEND_PRIVATE_KEY : `0x${process.env.BACKEND_PRIVATE_KEY}`;
        const rpcUrl = process.env.SEPOLIA_RPC_URL;
        const vaAddr = process.env.RWA_VERIFIED_ASSETS;
        if (!pk || pk.length < 10)
        ...
        const account = privateKeyToAccount(pk);
        const wallet = createWalletClient({ account, chain: sepolia, transport: http(rpcUrl) });
        const hash = await wallet.writeContract({ address: vaAddr, abi: rwaVerifiedAssetsAbi, functionName: 'DeRegisterVerifiedAsset', args: [owner, BigInt(assetId)] });
...
app.post('/assets/update', async (req, res) => {
    try {
        const { owner, assetId, newValueUSD, isLocked, tradable } = req.body;
        if (!owner || !assetId || !newValueUSD)
            return res.status(400).json({ error: 'owner, assetId, newValueUSD required' });
        ...
        const account = privateKeyToAccount(pk);
        const wallet = createWalletClient({ account, chain: sepolia, transport: http(rpcUrl) });
        const hash = await wallet.writeContract({ address: vaAddr, abi: rwaVerifiedAssetsAbi, functionName: 'upDateAssetValue', args: [owner, BigInt(assetId), BigInt(newValueUSD), lock, trad] });
        res.json({ txHash: hash });
    }
    catch (e) {
...
app.post('/mint/proof', async (req, res) => {
    try {
        const { owner, requestId } = req.body;
        ...
        if (!pk || pk.length < 10)
            throw new Error('Missing BACKEND_PRIVATE_KEY');
        const account = privateKeyToAccount(pk);
        const abiBytes = encodeAbiParameters(parseAbiParameters('address,uint256'), [owner, BigInt(requestId)]);
...
app.get('/debug/manager/preflight', async (req, res) => {
    try {
        ...
        if (!requestId)
            return res.status(400).json({ error: 'requestId required' });
        const rpcUrl = process.env.SEPOLIA_RPC_URL;
        const managerAddr = process.env.RWA_MANAGER;
        const vaAddr = process.env.RWA_VERIFIED_ASSETS;
        const nftAddr = process.env.RWA_NFT;
        const coinAddr = process.env.RWA_COINS || process.env.RWA_Coins;
        if (!rpcUrl || !managerAddr || !vaAddr || !nftAddr)
        ...
        const memberRole = await pub.readContract({ address: managerAddr, abi: managerAccessAbi, functionName: 'MEMBER_ROLE' });
        const isBackendMember = await pub.readContract({ address: managerAddr, abi: managerAccessAbi, functionName: 'hasRole', args: [memberRole, backend.address] });
        // 2) VerifiedAssets checks: under owner vs backend
        const assetsOwner = (await pub.readContract({ address: vaAddr, abi: rwaVerifiedAssetsAbi, functionName: 'getVerifiedAssets', args: [owner] }));
        const assetsBackend = (await pub.readContract({ address: vaAddr, abi: rwaVerifiedAssetsAbi, functionName: 'getVerifiedAssets', args: [backend.address] }));
        const foundUnderOwner = (assetsOwner || []).some((a) => a.assetId === BigInt(requestId));
        const foundUnderBackend = (assetsBackend || []).some((a) => a.assetId === BigInt(requestId));
        // 3) NFT owner is manager?
        const nftOwner = await pub.readContract({ address: nftAddr, abi: rwaNftAbi, functionName: 'owner' });
        const isNftOwnerManager = nftOwner.toLowerCase() === managerAddr.toLowerCase();
        // 4) Coin minter role
        let coinMinterOk = null;
        try {
            if (coinAddr) {
                const role = await pub.readContract({ address: coinAddr, abi: accessControlAbi, functionName: 'MINTER_ROLE' });
                coinMinterOk = await pub.readContract({ address: coinAddr, abi: accessControlAbi, functionName: 'hasRole', args: [role, managerAddr] });
            }
        }
        catch {
            coinMinterOk = null;
...
app.get('/debug/asset/:registryKey/:assetId', async (req, res) => {
    try {
        const { registryKey, assetId } = req.params;
        if (!registryKey || !assetId)
            return res.status(400).json({ error: 'registryKey and assetId required' });
        const rpcUrl = process.env.SEPOLIA_RPC_URL;
        const vaAddr = process.env.RWA_VERIFIED_ASSETS;
        if (!rpcUrl || !vaAddr)
            throw new Error('env missing');
        ...
        const found = assets.find((a) => a.assetId === BigInt(assetId));
        if (!found) {
            return res.json({ error: 'Asset not found', registryKey, assetId, totalAssets: assets.length });
        }
        // Normalize for JSON
        const normalized = {
...
app.get('/manager/status', async (req, res) => {
    try {
        const user = req.query.user || '';
        const requestId = req.query.requestId || '';
        if (!user || !/^0x[0-9a-fA-F]{40}$/.test(user))
            return res.status(400).json({ error: 'valid user query param required' });
        if (!requestId)
            return res.status(400).json({ error: 'requestId required' });
        const rpcUrl = process.env.SEPOLIA_RPC_URL;
        const managerAddr = process.env.RWA_MANAGER;
        if (!rpcUrl)
            throw new Error('Missing SEPOLIA_RPC_URL');
        if (!managerAddr)
            throw new Error('Missing RWA_MANAGER');
        const pub = createPublicClient({ chain: sepolia, transport: http(rpcUrl) });
        const info = await pub.readContract({ address: managerAddr, abi: rwaManagerAbi, functionName: 'getUserAssetInfo', args: [user, BigInt(requestId)] });
...
app.get('/assets/list', async (req, res) => {
    try {
        const owner = req.query.owner || '';
        if (!owner || !/^0x[0-9a-fA-F]{40}$/.test(owner))
            return res.status(400).json({ error: 'valid owner query param required' });
        const rpcUrl = process.env.SEPOLIA_RPC_URL;
        const vaAddr = process.env.RWA_VERIFIED_ASSETS;
        if (!rpcUrl)
            throw new Error('Missing SEPOLIA_RPC_URL');
        if (!vaAddr)
            throw new Error('Missing RWA_VERIFIED_ASSETS');
        const pub = createPublicClient({ chain: sepolia, transport: http(rpcUrl) });
        const data = await pub.readContract({ address: vaAddr, abi: rwaVerifiedAssetsAbi, functionName: 'getVerifiedAssets', args: [owner] });
...
app.get('/assets/member', async (req, res) => {
    try {
        const addr = req.query.addr || '';
        if (!addr || !/^0x[0-9a-fA-F]{40}$/.test(addr))
            return res.status(400).json({ error: 'valid addr query param required' });
        const rpcUrl = process.env.SEPOLIA_RPC_URL;
        const vaAddr = process.env.RWA_VERIFIED_ASSETS;
        if (!rpcUrl)
            throw new Error('Missing SEPOLIA_RPC_URL');
        if (!vaAddr)
            throw new Error('Missing RWA_VERIFIED_ASSETS');
        const pub = createPublicClient({ chain: sepolia, transport: http(rpcUrl) });
        const allowed = await pub.readContract({ address: vaAddr, abi: rwaVerifiedAssetsAbi, functionName: 'isOwnerAllowed', args: [addr] });
        res.json({ allowed });
    }
    catch (e) {
...
app.post('/assets/member/add', async (req, res) => {
    try {
        const { addr } = req.body;
        if (!addr || !/^0x[0-9a-fA-F]{40}$/.test(addr))
            return res.status(400).json({ error: 'valid addr required' });
        const pk = process.env.BACKEND_PRIVATE_KEY?.startsWith('0x') ? process.env.BACKEND_PRIVATE_KEY : `0x${process.env.BACKEND_PRIVATE_KEY}`;
        const rpcUrl = process.env.SEPOLIA_RPC_URL;
        const vaAddr = process.env.RWA_VERIFIED_ASSETS;
        if (!pk || pk.length < 10)
        ...
        const wallet = createWalletClient({ account, chain: sepolia, transport: http(rpcUrl) });
        const pub = createPublicClient({ chain: sepolia, transport: http(rpcUrl) });
        const txHash = await wallet.writeContract({ address: vaAddr, abi: rwaVerifiedAssetsAbi, functionName: 'addMember', args: [addr] });
...
app.get('/assets/registry', async (_req, res) => {
    try {
        const pk = process.env.BACKEND_PRIVATE_KEY?.startsWith('0x') ? process.env.BACKEND_PRIVATE_KEY : `0x${process.env.BACKEND_PRIVATE_KEY}`;
        if (!pk || pk.length < 10)
            throw new Error('Missing BACKEND_PRIVATE_KEY');
...
app.get('/nft/tokensOf', async (req, res) => {
    try {
        const owner = req.query.owner || '';
        if (!owner || !/^0x[0-9a-fA-F]{40}$/.test(owner))
            return res.status(400).json({ error: 'valid owner query param required' });
        ...
        if (!rpcUrl)
            throw new Error('Missing SEPOLIA_RPC_URL');
        if (!nftAddr)
            throw new Error('Missing RWA_NFT');
        const pub = createPublicClient({ chain: sepolia, transport: http(rpcUrl) });
        const head = await pub.getBlockNumber();
        const range = 200000n; // adjust if needed
        const fromBlock = head > range ? head - range : 0n;
        const transferEvent = parseAbiItem('event Transfer(address indexed from, address indexed to, uint256 indexed tokenId)');
        const logs = await pub.getLogs({ address: nftAddr, fromBlock, toBlock: head, event: transferEvent, args: { to: owner } });
        const seen = new Set();
        const candidateIds = logs.map((l) => (l.args?.tokenId ? l.args.tokenId.toString() : '0')).filter((s) => s !== '0');
